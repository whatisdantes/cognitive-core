...
class EpisodicMemory:
    ...
    def save(self, path: Optional[str] = None):
        ...
        print(f"💾 Эпизодическая память сохранена: {len(self._episodes)} эпизодов → {path}")
    ...
    def _load(self):
        ...
        try:
            ...
            print(f"📂 Эпизодическая память загружена: {len(self._episodes)} эпизодов ← {self._data_path}")
        except Exception as e:
            print(f"⚠ Ошибка загрузки эпизодической памяти: {e}")
