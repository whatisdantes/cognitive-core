...
class SourceMemory:
    ...
    def blacklist(self, source_id: str, reason: str = ""):
        ...
        self._write_count += 1
        self._maybe_autosave()
        print(f"🚫 Источник занесён в чёрный список: '{source_id}' — {reason}")
    ...
    def save(self, path: Optional[str] = None):
        ...
        print(f"💾 Память об источниках сохранена: {len(self._sources)} источников → {path}")
    ...
    def _load(self):
        ...
        try:
            ...
            print(f"📂 Память об источниках загружена: {len(self._sources)} источников ← {self._data_path}")
        except Exception as e:
            print(f"⚠ Ошибка загрузки памяти об источниках: {e}")
