...
class WorkingMemory:
    ...
    def search(
        ...
        all_items = self.get_all()  # already returns a copy under lock
        results = []
    ...
    def _adaptive_max_size(self) -> int:
        ...
        try:
            ...
            if ram_pct > self._ram_limit_pct * 0.85:
    ...
    def ram_usage_mb(self) -> float:
        ...
        try:
            import sys
            all_items = self.get_all()  # returns copy under lock
            total = sum(sys.getsizeof(item.content) for item in all_items)
    ...
    def status(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                "type": "working_memory",
                "size": len(self._items) + len(self._protected),
    ...
    def __repr__(self) -> str:
        with self._lock:
            return (
                f"WorkingMemory(size={len(self._items) + len(self._protected)}/{self._max_size} | "
                f"protected={len(self._protected)} | "
                f"evicted={self._evict_count})"
            )

    def __iter__(self):
        return iter(self.get_all())  # snapshot under lock via get_all()
