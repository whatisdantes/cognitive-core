...
class SemanticMemory:
    ...
    def save(self, path: Optional[str] = None):
        ...
        print(f"💾 Семантическая память сохранена: {len(self._nodes)} понятий → {path}")
    ...
    def _load(self):
        ...
        try:
            ...
            print(f"📂 Семантическая память загружена: {len(self._nodes)} понятий ← {self._data_path}")
        except Exception as e:
            print(f"⚠ Ошибка загрузки семантической памяти: {e}")
    ...
    def _evict_least_important(self):
        ...
        scored = [
            (n.importance * n.confidence * math.log(n.access_count + 1 + 1), k)
